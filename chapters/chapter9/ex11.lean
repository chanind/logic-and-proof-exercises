...
theorem t1 : x - x = 0 :=
calc
x - x = x + -x   : sub_eq_add_neg x x
    ... = 0      : add_right_neg x

theorem t2 (h : x + y = x + z) : y = z :=
calc
y       = 0 + y        : eq.symm(zero_add y)
    ... = (-x + x) + y : by rw add_left_neg
    ... = -x + (x + y) : by rw add_assoc
    ... = -x + (x + z) : by rw h
    ... = (-x + x) + z : by rw add_assoc
...
x       = x + 0        : by rw [add_comm, zero_add]
    ... = x + (y + -y) : by rw [add_comm y, add_left_neg y]
    ... = (x + y) + -y : by rw add_assoc
    ... = (z + y) + -y : by rw h
    ... = z + (y + -y) : by rw add_assoc
    ... = z + 0        : by rw [add_comm y, add_left_neg]
    ... = z            : by rw [add_comm, zero_add]
...
calc
    x * 0 + x * 0 = x * (0 + 0) : by rw left_distrib
            ... = x * 0       : by rw zero_add
            ... = x * 0 + 0   : by rw [add_comm, zero_add],
show x * 0 = 0, from t2(x * 0)(x * 0)(0)(h1)
